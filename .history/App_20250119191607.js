  ...
  useEffect(() => {
    ...
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        // Only update user location if it's different
        setUserLocation({
          latitude,
          longitude,
          latitudeDelta: 0.01,
        ...
        // Update the region with the user's location, but only once
  ...
  const BottomSheet = () => {
    return (
      <View style={styles.container}>
        <MapView
          style={styles.map}
          region={region} // This should not keep changing unnecessarily
          ...
          onRegionChangeComplete={(newRegion) => {
            // Optionally update the region on user drag, but avoid constant re-renders
