...
const App = () => {
  ...
  const [region, setRegion] = useState(annaNagarLocation); // Initial region set to Anna Nagar
  ...
  useEffect(() => {
    Geolocation.getCurrentPosition(
      (position) => {
        ...
        // Update region only once the user's location is fetched
        setRegion({
          latitude,
          longitude,
          latitudeDelta: 0.01,
  ...
  const BottomSheet = () => {
    return (
      <View style={styles.container}>
        <MapView
          style={styles.map}
          region={region} // Use the region from the user's location
          showsUserLocation={true}
          showsMyLocationButton={true}
          scrollEnabled={true}  // Enable map scrolling
          zoomEnabled={true}    // Enable zoom
        >
          {/* If user location is available, show marker */}
          {userLocation && (
